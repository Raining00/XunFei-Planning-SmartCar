...
1. 针对地图C区域的视觉死角,做了一些特定的调整动作，保证能够涵盖到C区所有障碍物参数；
2. 针对视觉的改动进行了脚本适配；
3. 进行了新的档位调整,如果能够规划成功,将会得到非常漂亮和稳定的轨迹,但是因为雷达畸变的存在,非常有可能导致代价地图被封死,因此存在风险,只能从尽可能提高定位稳定性尤其是旋转的噪声上进行下手,这将会是一个非常漂亮的档位；
4. 加入了辅助停车,但存在辅助停车时机有时候不太对的情况,需要对此进行细化；
5. 修改了获取二维码的方式为调用服务；
6. 目前依旧存在换档的时候会导致局部规划节点闪退的情况, 还需进行调整.

#### --7.9 update--

1. 加入了语音控制成部分.可以根据语音命令进行C-Siwing动作；
2. 修改了停车位测试脚本, 可以从终端传入参数选择停车位置；
3. 修改了任务调度脚本,进行了规则适配,可以从终端传入参数决定停车位置和播报种类；

#### --7.12 update --

1. 重新加入了异常状态检测功能, 改进了以前的方法,考虑到回调函数不进入就无法判断,并且触发amcl_pose更新需要小车进行移动,因此改用外部定时器, 目前测试较为稳定；
2.  修改了一些档位的参数, 同时适配了全局避障和一定的速度要求, 目前对于180度转向的弯道由于雷达无法扫到另一面是否有障碍,因此当另一面有障碍的时候,无法即时避开障碍物；
3.  加入了激光雷达对于障碍物的分割,目前采用欧几里德聚类分割,搜索采用Kdtree, 分割以及距离阈值依然需要调整,目前打算利用该方法来进行高低速档位的切换.


#### --7.18 update--

1. 档位优化,目前暂定四个档位,2号档位不稳定,暂不可用,1档和4档,5档为稳定档位
2. 1档优化,降低了膨胀半径,进入窄道后进行变档加大了膨胀半径；

#### --7.27 update--
1. 抛弃了全国选拔赛中的导航调度逻辑,正式开始备赛全国总决赛,为每一个导航点加上了自己的类型,目前分为距离判定形,角度判定型,转向回正型,B区域任务领取型以及停车导航点,
2. 加入了全图枚举拍照逻辑,区分了全拍和不拍盲区的两个方法；
3. 加入了激光雷达障碍物探测,可在实时检测的情况下,自动判别盲区是否有障碍物,来决定是否对其进行检测；对于贴墙障碍物没法使用雷达的方式去进行检测,目前无法从车上已有的传感器中找到探测盲区贴墙障碍物的方法；

#### --8.5 update--
1. 更改了方案为识别拍照,解决了导航途中更改导航目标为朝向人物模型后,无法继续运动的情况,本质原因在与计算yaw与目标yaw之间的差值由于没有统一区间而导致的问题；
2. 调整了对于速度控制上的参数,不同于实时检测,需要对角速度进行提升,否则容易出现转向能力不够,容易擦到地图板子的情况；
3. 对于定点拍照,依旧保留此方法.

#### --8.9 update--
1. 重新将华北赛的档位加到了正式的档位中, 并且导航点重新增加了一个,考虑到后续,可能还会增加一个导航点, 目前没有问题,暂时不加.在旋转较大的弯道处将会短暂停留一会；
2. 对于拍照的方案,在C区域不接受视觉和雷达检测的任何请求,C区改为华为赛方案.其他区域正常接受服务请求；
3. 优化了脚本逻辑和复杂度,修改了一些bug；

<<<<<<< HEAD

#### --8.15 update--
1. 初步定下低速速度控制档位,策略方式待定；
2. 初步定下高速速度控制档位,策略方式待定；
