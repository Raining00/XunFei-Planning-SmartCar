...
class LaserDetection {
 public:
  LaserDetection() : _context(1), _clientSocket(_context, ZMQ_REQ) {
    ...
    nh.param<double>("change_segment_distance_threshold", change_segment_distance_threshold, 0.02);
    nh.param<double>("cluster_tolerance", cluster_tolerance, 0.1);
    nh.param<int>("min_cluster_size", min_cluster_size, 8);
    nh.param<double>("detection_threshold", detection_threshold, 1.2);
  ...
  void ChangeMode(const geometry_msgs::TwistConstPtr &msg);
 ...
 private:
  ...
  double segment_distance_threshold, change_segment_distance_threshold, cluster_tolerance, detection_threshold;
