...
class LaserDetection {
 public:
  LaserDetection() : _context(1), _clientSocket(_context, ZMQ_REQ) {
    ...
    points_pub_ = nh.advertise<geometry_msgs::PoseArray>("obstacles", 10);
    ...
    // node params
    nh.param<double>("inlier_distance_threshold", inlier_distance_threshold, 0.04);
    nh.param<double>("outlier_distance_threshold", outlier_distance_threshold, 0.04);
    nh.param<double>("cluster_tolerance", cluster_tolerance, 0.2);
    nh.param<int>("min_cluster_size", min_cluster_size, 4);
  }
  void LoadMap();
 ...
 private:
  ...
  ros::Publisher enemy_pub_, seg_pc_pub_, in_map_pub_, points_pub_, marker_pub_;
  Pose estimate_pose_;
  Point self_pose_;
  bool is_initialized_tracking_, is_first_detection;
  ...
  double inlier_distance_threshold, outlier_distance_threshold, cluster_tolerance;
