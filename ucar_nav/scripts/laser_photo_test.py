...
class mission_task:
        ...
        if self.state == 0:
            ...
            if self.goal_type == 1:  # 区域切换导航点
                ...
                if dyaw < 0.2:
                    ...
                    self.person_client()
                    self.state = 2 if self.person_num >= 2 else 21
            ...
            if distance < 0.25:
                ...
                self.current_goal.pose.orientation.z = parking_reverse_dict[self.parking_id][0]
                self.current_goal.pose.orientation.w = parking_reverse_dict[self.parking_id][1]
                self.goal_pub.publish(self.current_goal)
                self.state = 2
        elif self.state == 2:
            if dyaw <= 0.15 and self.person_num < 2 or distance < 0.25:
    ...
    def person_client(self):
        ...
        try:
            ...
            print("glass", self.personNum.linear.x, "long", self.personNum.linear.y, "person", self.personNum.linear.z)
