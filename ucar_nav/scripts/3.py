...
# 5: successfully parking, start to broadcast, specially, we will wait for five second to make sure detecting result can be recived.
 # 6: finishing
class mission_task:
    def __init__(self, goal_path, parking_path, parking_id,is_gear_1):
        # nav point
        self.nav_list = []
        self.parking_list = []
        ...
        self.person_num = random.randint(0, 2)
        ...
        self.is_gear_1 = is_gear_1
        self.min_distance = [0.70, 0.15, 0.12, 0.1] if self.is_gear_1 else [0.25,0.15,0.12,0.1]
        self.can_change = 0
    ...
    def Run(self):
        ...
        if self.state == 0:
            ...
            if distance < self.min_distance[self.goal_id-1]:
                if self.is_gear_1 and self.goal_id == 1:
                    temp_msg = Twist()
                    temp_msg.linear.x = 2
            ...
            if self.swing_head == True:
                ...
                if abs(yaw_goal - yaw) < 0.15 or distance < 0.1:
                    temp_msg = Twist()
                    temp_msg.linear.x = 1
                    temp_msg.linear.y = 1
                    ...
                    print("yaw-diff",abs(yaw_goal - yaw))
            elif distance < 0.1:
            ...
            if abs(yaw_goal - yaw) < 0.15:
                ...
                if self.parking_id == 2:
                    self.current_goal.pose.orientation.z = 0.707
                    self.current_goal.pose.orientation.w = 0.707
                    self.current_goal.header.stamp = rospy.Time.now()
                    self.goal_pub.publish(self.current_goal)
                    self.state = 41
                else:
            ...
            self.assist_parking_pub.publish(vel_msg)
            self.person_client()
            rospy.loginfo("人数:%d, 长头发:%d, 戴眼镜:%d",
                          self.person_num, self.longhair_num, self.glasses_num)
    ...
    def person_client(self):
        ...
        try:
            ...
            if self.personNum.linear.x > 20:
                self.glasses_num = 20
            else:
                self.glasses_num = int(self.personNum.linear.x)

            if self.personNum.linear.y > 20:
                self.longhair_num = 20
            else:
                self.longhair_num = int(self.personNum.linear.y)

            if self.personNum.linear.z > 20:
                self.person_num = 20
            else:
                self.person_num = int(self.personNum.linear.z)
    ...
    def broadcast(self):
        if self.glasses_num < 20 and self.longhair_num < 20:
            if self.state != 7:
                ...
                #soundname = str(self.person_num) + str(self.longhair_num) + str(self.glasses_num)
                #playsound('/home/ucar/voice_ws/src/voice_library/' + soundname +'.wav')

                playsound('/home/ucar/voice_ws/src/voice_library/D' + str(int(self.food_id))+'.wav')
    ...
    def voiceCallback(self, msg):
        if msg.linear.x == 3:
            ...
            self.assist_parking_pub.publish(vel)
            time.sleep(1)
...
if __name__ == '__main__':
    try:
        ...
        goal_path = os.getcwd() + "/2.csv"
        parking_path = os.getcwd()+'/parking.csv'
        if len(sys.argv) >= 3:
            if int(sys.argv[2]):
                print("Gear1 and change mode")
            mission_task(goal_path, parking_path, int(sys.argv[1]), int(sys.argv[2]))
        else:
            mission_task(goal_path, parking_path, int(sys.argv[1]), 0)
        rospy.spin()
