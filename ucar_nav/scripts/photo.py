...
class mission_task:
            ...
            if self.goal_type == 1:  # 区域切换导航点
                ...
                if dyaw < 0.4:
                    if self.goal_id == len(self.nav_list):
                        ...
                        temp_msg.linear.x = 1
                        self.change_pub.publish(temp_msg)
                        self.pub_next_goal(is_parking=True,with_photo = False)
                        self.state = 2
                    else:
                        self.pub_next_goal(with_photo=False)
        elif self.state == 2:
            if distance < 0.25:
    ...
    def pub_next_goal(self, is_parking=False, with_photo=True):
        if with_photo:
            temp_msg = Twist()
            temp_msg.linear.y = 1
            self.change_pub.publish(temp_msg)
    ...
    def obsHandle(self,req):
        ...
        while (yaw >= math.pi or yaw < -math.pi):
            if yaw >= math.pi:
                yaw -= math.pi*2
            elif yaw< -math.pi:
                yaw += math.pi*2
        ...
        self.current_goal.pose.orientation.z = yaw[2]
        self.current_goal.pose.orientation.w = yaw[3]
    ...
    def voiceCallback(self, msg):
        if msg.linear.x == 2:
            self.nav_list = []
            self.goal_type_list = []
            with open('photo_without.csv', 'rb') as f:
                reader = csv.reader(f)
                for cols in reader:
                    self.nav_list.append([float(values) for values in cols])
                    self.goal_type_list.append(int(cols[-1]))
...
if __name__ == '__main__':
    try:
        ...
        goal_path = os.getcwd() + "/photo_with.csv"
